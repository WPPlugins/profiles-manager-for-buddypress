...
function bp_restrict_profile_head() { 
	...
	$user_role = $user->roles[0];
        ...
        $pub_hid = $wpdb->get_results("SELECT group FROM bpm_profiles WHERE p_hidden = 'hidden' AND level = '$user_role'");
	
	echo '<style type="text/css">';
	
	foreach ($pub_hid as $pitem) {
	echo 'div.' . $pitem->bb_group . ' { display: none; }';
	}
	...
	$mem_hid = $wpdb->get_results("SELECT group FROM bpm_profiles WHERE m_hidden = 'hidden' AND level = '$user_roles'");
	
	echo '<style type="text/css">';
	
	foreach ($mem_hid as $mitem) {
	echo 'form.' . $mitem->bb_group . ' { display: none; }';
	echo 'li.' . $mitem->bb_group . ' { display: none; }';
	}
	...
	$message = $wpdb->get_var($wpdb->prepare("SELECT option_value FROM bpm_settings WHERE option_name = 'message';"));
	
	//Get an array of public hidden profile items.
	
	$list = array();
	$pro_ite = $wpdb->get_results("SELECT * FROM bpm_profiles WHERE p_hidden = 'hidden' AND level = '$user_roles'");
...
function bp_pm_group_tabs() {
	...
	$user_roles = $current_user->roles[0];
        ...
        $str = $wpdb->get_results("SELECT group FROM bpm_profiles WHERE m_hidden='shown' AND level='$user_roles'");
	...
	for ( $i = 0; $i < count($groups); $i++ ) {
		...
		foreach ( $str as $hide) {
		if ($hide->bb_group == sanitize_title($groups[$i]->name)) {
	...
	$pro_ite = $wpdb->get_results("SELECT * FROM bpm_profiles WHERE p_hidden = '%s' AND level = '%s'", "hidden", $user_roles);
